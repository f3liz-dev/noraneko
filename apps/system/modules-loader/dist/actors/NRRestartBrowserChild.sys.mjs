//#region ../../modules/actors/NRRestartBrowserChild.sys.mts
var NRRestartBrowserChild = class extends JSWindowActorChild {
	actorCreated() {
		console.debug("NRRestartBrowserChild created!");
		const window = this.contentWindow;
		if (window?.location.port === "5183") {
			console.debug("NRRestartBrowser 5183!");
			Cu.exportFunction(this.NRRestartBrowser.bind(this), window, { defineAs: "NRRestartBrowser" });
		}
	}
	NRRestartBrowser(callback = () => {}) {
		const promise = new Promise((resolve, _reject) => {
			this.resolveRestartBrowser = resolve;
		});
		this.sendAsyncMessage("RestartBrowser:Restart");
		promise.then((result) => callback(result));
	}
	resolveRestartBrowser = null;
	async receiveMessage(message) {
		switch (message.name) {
			case "RestartBrowser:Result": {
				this.resolveRestartBrowser?.(message.data);
				this.resolveRestartBrowser = null;
				break;
			}
		}
	}
};

//#endregion
export { NRRestartBrowserChild };