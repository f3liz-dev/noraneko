...
Deno.test("RPC Registry - Call non-existent module should timeout", async () => {
  await assertRejects(
    async () => {
...
Deno.test("RPC Registry - Try call non-existent module should return undefined", async () => {
...
Deno.test("RPC Registry - Use soft proxy (doesn't throw)", async () => {
  ...
  assertEquals(result, undefined, "Soft proxy should return undefined for missing module");
});
...
Deno.test("RPC Registry - Pending calls are processed", async () => {
  ...
  assertEquals(result, "delayed-data", "Pending call should resolve after module registration");
